      ...
      <View style={[this.props.containerStyle, {height: height}]}>
        ...
        >
          {this.props.dataSource.map ((image, index) => {
            const imageObject = typeof image.url === 'string'
              ? {uri: image.url.substr (0, image.url.indexOf (','))}
              : image.url;
            const textComponent = (
              <View style={styles.layoutText}>
                ...
                {image.type === undefined
                  ...
                  : <Text style={styles.textType}>
                      {image.type === 'ItemBase' && 'Từ dịch vụ vừa dùng'}
                      {image.type === 'Promo' && 'Khuyến mãi'}
                      {image.type === 'UserBase' && 'Từ người dùng giống bạn'}
                    </Text>}
              </View>
            );
